...
class VideoGeneratorWASM {
    ...
    // 🔧 زيادة الانتظار بعد seek
    async settleAfterSeek() { 
        await this._rAF();
        await this._rAF();
        await this._rAF(); // فريم إضافي
        await new Promise(r => setTimeout(r, 50)); // 50ms إضافية
    }
    ...
    async captureFrameBytes(previewElement, canvas, ctx, width, height, frameIndex) {
        ...
        try {
            ...
            const captureCanvas = await html2canvas(targetElement, {
                ...
                useCORS: true,
                allowTaint: false,
                backgroundColor: '#000000', // 🔧 لون خلفية واضح بدلاً من null
                logging: false,
                imageTimeout: 0,
            ...
            const blob = await new Promise(res => canvas.toBlob(res, 'image/jpeg', 0.90)); // 🔧 جودة أعلى
    ...
    async renderMP4Deterministic({
        ...
        try {
            ...
            if (iframeWin?.setCaptureMode) {
                ...
                await new Promise(r => setTimeout(r, 100)); // انتظار تطبيق التغييرات
            }
            ...
            for (let part = 0; part < partCount; part++) {
                ...
                await this.ffmpeg.exec([
                    ...
                    '-crf', String(crf),
                    '-preset', preset,
                    '-tune', 'film', // 🔧 film أفضل من animation للمحتوى المختلط
                    '-movflags', '+faststart',
